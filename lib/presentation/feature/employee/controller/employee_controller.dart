...
class EmployeeController extends GetxController with StateMixin<List<EmployeeProfile>>{
  ...
  EmployeeController({required EmployeeUseCase employeeUseCase}):_employeeUseCase = employeeUseCase;
  ...
  Future<void> getEmployeeProfiles() async{
    ...
    switch(result.requestStatus){
      ...
      case RequestStatus.failed:
        change(employeeProfiles,status: RxStatus.error(result.errorResponse!.error!));
        break;
      case RequestStatus.somethingWentWrong:
        change(employeeProfiles,status: RxStatus.error("Something went wrong"));
        break;
    }
  }
